...
def profile():
    ...
    from sqlalchemy.orm import joinedload
    user = db.query(User).options(joinedload(User.orders).joinedload(Order.items)).filter(User.id == session["user_id"]).first()
    ...
    if request.method == "POST":
        ...
        if phone:
            ...
            if existing:
                db.close()
                flash("Bu telefon raqam boshqa foydalanuvchida bor", "danger")
